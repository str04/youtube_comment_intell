...
with st.sidebar:
    st.markdown(
        "<div style='font-size:1.4rem;font-weight:700;padding:1rem 0 1.5rem'>🎯 YT Intel</div>",
        unsafe_allow_html=True,
    )
...
st.markdown(
    "<div style='color:var(--muted);margin-bottom:2rem'>"
    "YouTube Comment Intelligence — a Data Science capstone project"
    "</div>",
    unsafe_allow_html=True,
)
...
├── ai/
│   ├── langchain_summary.py      ← Groq LLaMA3 creator report
│   └── content_gap_finder.py     ← Groq LLaMA3 gap analysis
└── dashboard/
    ├── app.py                    ← Home
    └── pages/
        ├── 1_analyze.py          ← Full analysis page
        └── 2_about.py            ← This page
""", language="text")
...
models = [
    ...
    (
        "LLM Report",
        "Groq · LLaMA3",
        "creator summaries and gap analysis",
    ),
]
...
steps = [
    (
        "1",
        "Fetch",
        "yt-dlp pulls comments and metadata from any public YouTube URL.",
    ),
    (
        "2",
        "Parse",
        "Remove URLs, normalize whitespace, demojize, deduplicate, and filter empty comments.",
    ),
    (
        "3",
        "Language",
        "Detect language and route Hindi, English, and Hinglish comments to the correct model.",
    ),
    (
        "4",
        "NLP Batch",
        "Sentiment, emotion, toxicity, and sarcasm run in batched inference.",
    ),
    (
        "5",
        "Intent",
        "Rule-based classifier tags each comment as question, praise, complaint, or suggestion.",
    ),
    (
        "6",
        "Topics",
        "BERTopic clusters comment text into themes with keyword labels.",
    ),
    (
        "7",
        "Segments",
        "KMeans clusters comments into audience personas using NLP feature vectors.",
    ),
    (
        "8",
        "Score",
        "Quality scorer ranks comments by length, likes, sentiment, and toxicity penalty.",
    ),
    (
        "9",
        "AI Report",
        "Enriched analysis JSON is sent to Groq LLaMA3 for creator summaries and content gaps.",
    ),
    (
        "10",
        "Cache",
        "Full analysis is stored in MongoDB so repeat URLs can load faster.",
    ),
]
...
<div class='yt-card' style='border-left:3px solid var(--yellow);margin-top:1rem'>
    <b>Note:</b> If the dashboard currently uses mock data, replace
    <code style='background:var(--surface2);padding:2px 6px;border-radius:4px'>run_mock_analysis()</code>
    in <code>pages/1_analyze.py</code> with calls to your real
...
st.markdown(
    "<div style='color:var(--muted);font-size:.82rem;text-align:center'>"
